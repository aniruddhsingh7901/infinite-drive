...
//     async registerWebhook(address: string, currency: string): Promise<void> {
...
//             event: 'confirmed-tx',
//             address,
//             url: `${config.apiUrl}/hooks?${config.apiKey}`,
//             confirmations: config.minConfirmations,
//             token: config.apiKey
//         };

//         await axios.post(`${config.apiUrl}/hooks?${config.apiKey}`, webhookData);
//     }
...
export class BlockchainService {
    ...
    async registerWebhook(address: string, currency: string, orderId: string): Promise<void> {
        ...
        const webhookData = {
            event: 'confirmed-tx',
            address,
            url: `https://d840-2409-40c4-302a-1c2c-44d8-3dd2-ff2b-e830.ngrok-free.app/webhooks/blockcypher?orderId=${orderId}`,
            token: config.apiKey
        };
